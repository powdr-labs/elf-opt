...
theorem exec_bne (s : State) (rs1 rs2 : Reg) (imm : UInt32) :
    exec s (Instr.bne rs1 rs2 imm)
    = if getReg s rs1 != getReg s rs2 then jumpTo s (s.pc + imm) else advance s := rfl

theorem exec_beq (s : State) (rs1 rs2 : Reg) (imm : UInt32) :
    exec s (Instr.beq rs1 rs2 imm)
    = if getReg s rs1 == getReg s rs2 then jumpTo s (s.pc + imm) else advance s := rfl

theorem exec_bltu (s : State) (rs1 rs2 : Reg) (imm : UInt32) :
    exec s (Instr.bltu rs1 rs2 imm)
    = if getReg s rs1 < getReg s rs2 then jumpTo s (s.pc + imm) else advance s := rfl

/-! ## Jumps. -/

theorem exec_jal (s : State) (rd : Reg) (imm : UInt32) :
    exec s (Instr.jal rd imm) = jumpTo (setReg s rd (s.pc + 4)) (s.pc + imm) := rfl

theorem exec_jalr (s : State) (rd rs1 : Reg) (imm : UInt32) :
