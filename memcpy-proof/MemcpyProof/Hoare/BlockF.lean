...
def R_block_4byte : State → State → Prop :=
  fun s s' =>
    s.halted = false →
    let loaded : UInt32 := loadWord s (s.regs 14)
    s'.pc = s.pc + 16 ∧
    s'.halted = false ∧
    s'.haltAt = s.haltAt ∧
    s'.regs 11 = loaded ∧
    s'.regs 13 = s.regs 13 + 4 ∧
    s'.regs 14 = s.regs 14 + 4 ∧
    (∀ r, r ≠ 11 → r ≠ 13 → r ≠ 14 → s'.regs r = s.regs r) ∧
    s'.mem = (storeWord s (s.regs 13) loaded).mem
...
theorem block_4byte_triple : Triple block_4byte R_block_4byte := by
  ...
  -- All setReg/getReg lemmas have side conditions discharged by `decide`
  -- (literal register numbers) or by user-provided frame hypotheses.
  ...
  · simp [h13]
  · simp [h14]
  · intro r hr11 hr13 hr14; simp [hr11, hr13, hr14]
  · -- mem
    simp [h14, h13]
    unfold storeWord
    simp [storeByte]
