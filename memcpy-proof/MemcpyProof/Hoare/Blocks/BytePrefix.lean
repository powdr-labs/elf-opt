...
/-! ### Setup chunk: first 2 instrs of `bp_c1`. -/

def block_byte_prefix_setup : List Instr :=
  [ Instr.addi 15 11 1
  , Instr.addi 16 10 0
  ]

def R_block_byte_prefix_setup : State → State → Prop :=
  fun s s' =>
    let a0 := getReg s 10
    let a1 := getReg s 11
    s'.pc = s.pc + 8 ∧
...
theorem block_byte_prefix_setup_triple :
    Triple block_byte_prefix_setup R_block_byte_prefix_setup := by
  have h := (Triple_addi 15 11 1).append (Triple_addi 16 10 0)
  refine Triple.weaken h ?_
  rintro s s' ⟨_, rfl, rfl⟩
  have h_pc : s.pc + 4 + 4 = s.pc + 8 := by bv_decide
  simp [R_block_byte_prefix_setup, h_pc]
  intro r h15 h16
  simp [setReg, Ne.symm h15, Ne.symm h16]
...
/-- Algebraic decomposition: the full body splits as `setup ++ main`. -/
theorem block_byte_prefix_body_eq :
    block_byte_prefix_body = block_byte_prefix_setup ++ block_byte_prefix_main := by
  rfl
